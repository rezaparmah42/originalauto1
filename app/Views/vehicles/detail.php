...
<section class="section-shell"><div class="section-heading"><h2>خدمات مرتبط با این مدل</h2></div><div class="resource-links"><?php $serviceSlugs = ['diagnostic','electrical','engine-repair','automatic-transmission','periodic-service','ac-repair','suspension','brakes','battery','ecu','pre-purchase-inspection','emergency','airbag','car-air-filter-and-filters','oil-change']; foreach ($serviceSlugs as $serviceSlug): ?><a href="<?= SITE_URL ?>/services/<?= rawurlencode($serviceSlug) ?>/<?= rawurlencode((string) ($modelSlug ?: ($vehicle['slug'] ?? ''))) ?>"><?= e($serviceSlug === 'diagnostic' ? 'دیاگ تخصصی' : ($serviceSlug === 'electrical' ? 'تعمیر برق' : ($serviceSlug === 'engine-repair' ? 'تعمیر موتور' : ($serviceSlug === 'periodic-service' ? 'سرویس دوره‌ای' : ($serviceSlug === 'automatic-transmission' ? 'گیربکس اتوماتیک' : ($serviceSlug === 'oil-change' ? 'تعویض روغن' : 'خدمت مرتبط')))))) ?></a><?php endforeach; ?></div></section>
